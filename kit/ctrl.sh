...
#        ./crntb-adm.sh uninstall
        echo "Stopping kit.7x24hs.com service, begin killing ${PID}"
    ...
    if [ "$PID" != "" ] ; then
        echo "Starting kit.7x24hs.com service..."
        nohup python index.py -log_file_prefix=./log/kit.log &
        echo $! > pidfile    #record process id to file
        echo 'Startup kit.7x24hs.com service success!'
