...
#        ./crntb-adm.sh uninstall
        echo "Stopping qrcode.7x24hs.com service, begin killing ${PID}"
    ...
    if [ "$PID" != "" ] ; then
        echo "Starting qrcode.7x24hs.com service..."
        nohup python index.py -log_file_prefix=./log/qrcode.log &
        echo $! > pidfile    #record process id to file
        echo 'Startup qrcode.7x24hs.com service success!'
